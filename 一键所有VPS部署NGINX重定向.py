...
def update_server(name, hostname, port, username, password, domain):
    try:
        ...
        print(f"{name} 申请证书")
        #stdin, stdout, stderr = client.exec_command("curl https://get.acme.sh | sh && ~/.acme.sh/acme.sh --register-account -m xxxx@gmail.com && ~/.acme.sh/acme.sh --issue -d kjlion.gq --standalone")
        ...
        print(f"{name} 下载证书")
        #stdin, stdout, stderr = client.exec_command("~/.acme.sh/acme.sh --installcert -d kjlion.gq --key-file /home/nginx/certs/key.pem --fullchain-file /home/nginx/certs/cert.pem")
        ...
        # 重定向到那个网站
        redirect = "www.bilibili.com"
        stdin, stdout, stderr = client.exec_command('wget -O /home/nginx/nginx.conf https://raw.githubusercontent.com/kejilion/nginx/main/nginx.conf && sed -i "s/yuming.com/' + domain + '/g" /home/nginx/nginx.conf && sed -i "s/www.baidu.com/' + redirect + '/g" /home/nginx/nginx.conf')
        while not stdout.channel.exit_status_ready():
            if stdout.channel.recv_ready():
                print(stdout.channel.recv(1024).decode(), end="")
